(function ($) {
  ...
  $(document).ready(function () {
    ...
    function headerTitleWidthHeight() {
      ...
      let totleHeaderTitleWidth = parseInt(
        headerTitleWidth +
          bdTocWrapperPaddingValue +
          bdTocHeaderPaddingValue +
          3
      );
    ...
    $('.layout_toggle_button').on('click', function () {
      if (sticky_mode_position.sticky_sidebar_collapse_on_off == '1') {
        $('.bd_toc_container.scroll-to-fixed-fixed .bd_toc_wrapper').css(
          'opacity',
          '1'
        );
        if (bdTocContainer.hasClass('active')) {
    ...
    //progress bar
    if (
      typeof progress_bar_switcher !== 'undefined' &&
      progress_bar_switcher.progress_bar_switcher == 1
    ) {
  ...
  function widgetFLoatingOpen() {
    $(window).scroll(function () {
      ...
      if (
        windowScrollLength >
        $('.bd_toc_container').height() + $('.bd_toc_container').offset().top
      ) {
  ...
  $('.bd_toc_content_list_item').onePageNav({
    ...
    scrollChange: function ($currentListItem) {
      ...
      let current_list_item = $('.bd_toc_content_list_item ul li.current')
        .find('>:first-child')
        .text();
      $('.bd_toc_widget_floating_current_heading .current_list_item').text(
        current_list_item
      );

      if (
        handle.isProActivated &&
        widget_floating_option.widget_floating_option == '1'
      ) {
    ...
    $('.bd_toc_content_floating_list_item').onePageNav({
      ...
      scrollChange: function ($currentListItem) {
        ...
        let current_list_item = $(
          '.bd_toc_content_floating_list_item ul li.current'
        )
          .find('>:first-child')
          .text();
        $('.bd_toc_widget_floating_current_heading .current_list_item').text(
  ...
  if (
    handle.isProActivated &&
    widget_floating_content.widget_floating_content == 1
  ) {
    $('.current_list_item').hover(function () {
      $(this)
        ...
        .parent()
        .parent()
        .find('.bd_toc_floating_content')
        .addClass('widget_floating_content_open');
    ...
    let title_border_radius_top =
      widget_floating_content.title_border_radius_top + 'px';
    let title_border_radius_right =
      widget_floating_content.title_border_radius_right + 'px';
    let title_border_radius_bottom =
      widget_floating_content.title_border_radius_bottom + 'px';
    let title_border_radius_left =
      widget_floating_content.title_border_radius_left + 'px';
    ...
    if ($(window).width() <= 767) {
      $('.floating_toc_bg_overlay').on('click', function () {
        $('.bd_toc_floating_content').removeClass(
  ...
  if (
    handle.isProActivated &&
    widget_floating_content.widget_floating_position == 'bottom'
  ) {
    $('.current_list_item').hover(function () {
    ...
    if (
      handle.isProActivated &&
      widget_floating_content.widget_floating_content == 0
    ) {
      $('.current_list_item').hover(function () {
  ...
  if ($('.bd_toc_content_floating_list_item').length > 0) {
    $('.bd_toc_content_floating_list_item ul:first > li').addClass(
      'root_parent'
    );
  }
  ...
  $('.bd_toc_content_floating_list_item ul li a ').on('click', function () {
    let current_text = $('.bd_toc_content_floating_list_item ul li.current')
      .find('>:first-child')
      .text();
    $('.bd_toc_widget_floating_current_heading .current_list_item').text(
      current_text
  ...
  $('.bd_toc_widget_nav_prev').on('click', function () {
    let pre_id = '';
    let current_dom = $(
      '.bd_toc_floating_content .bd_toc_content_floating_list_item ul'
    ).find('li.current');
  ...
  function getFirstParentLi(element) {
    if (
      !element.parentNode ||
      element.parentNode.className === 'root_parent' ||
      element.parentNode.className === 'first root_parent'
    ) {
